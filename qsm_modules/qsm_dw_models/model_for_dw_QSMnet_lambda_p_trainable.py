...
class QSMnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv10 = nn.Sequential(nn.Conv3d(2,32,5,1,2),nn.BatchNorm3d(32),
        nn.LeakyReLU(0.1))
...
def toc():
    ...
    if 'startTime_for_tictoc' in globals():
        ...
        print("Time taking:",str(time.time() - startTime_for_tictoc) )
    else:
...
if __name__ == "__main__":
    net=QSMnet()
    net = net#.cuda(1)
    xx = torch.rand(1,2,176,176,160)#.cuda(1)
    tic()
    with torch.no_grad():
        yy = net(xx)
    toc()
