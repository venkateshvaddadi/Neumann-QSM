...
experiments_folder="../savedModels/Spinet_QSM_MODELS_dw_QSMnet_loss_l1_lambda_p_trainging/experiments_on_given_data/dw_WideResNet/single_patient/"
experiment_name="Dec_13_10_32_pm_model_K_3_given_single_patient_data_dw_WideResNet_patient_2//"
...
data_source='given_single_patient_data_4_fold';
...
if(strcmp(data_source,'given_single_patient_data_4_fold'))
    raw_data_path='../../QSM_data/data_for_experiments/given_data/raw_data_names_modified/'
...
if(strcmp(data_source,'given_single_patient_data_old'))
    raw_data_path='../../QSM_data/data_for_experiments/given_data/raw_data_names_modified/'
    test_case =[7,8,9,10,11,12]
end
...
for i=1:size(test_case,2)
    ...
    for j=1:5
        ...
        load(strcat(file_path,'phs',num2str(j),'.mat'));
        ...
        kz = (kz / max(abs(kz(:)))) / spatial_res(3);% Compute magnitude of kernel and perform fftshift
        k2 = kx.^2 + ky.^2 + kz.^2;
        ...
        for iter = 1:max_iter
            pHpy = real(ifftn(fftn(susc).*kernel.*kernel)).*single(msk);
            susc = susc + alpha * (pHy - pHpy);
